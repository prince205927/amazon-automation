...
public class AddToCartTest extends BaseTest {
	@Test(groups = { "cart", "smoke", "regression",
			"p0" }, priority = 1, description = "Verify add multiple items to cart from PDP")
	public void verifyMultipleCartDetailsFromPDP() {

        LoggerUtil.info("===== Test Started: verifyMultipleCartDetailsFromPDP =====");
        ExtentReportLogger.logStep("Starting Add to Cart test from PDP");
		...
		for (int i = 0; i < expectedCount && i < actualCount; i++) {
			...
			System.out.println("Expected: "
					+ expected.getTitle().substring(0, Math.min(60, expected.getTitle().length())) + "...");
			System.out.println(
					"Actual:   " + actual.getTitle().substring(0, Math.min(60, actual.getTitle().length())) + "...");
	...
	@Test(groups = { "cart", "regression",
			"p1" }, priority = 2, description = "Verify add multiple items to cart from product listings")
	public void verifyMultipleCartDetailsFromProductListings() {
		...
		softAssert.assertEquals(actualCount, expectedCount, "Cart item count mismatch!");
		...
		for (int i = 0; i < itemsToCheck; i++) {
			...
			softAssert.assertTrue(priceMatches, "Product " + (i + 1) + " price mismatch! Expected: $" + expectedPrice
					+ ", Actual: $" + actualPrice);
			System.out.println("Price: " + (priceMatches ? "✓ PASS" : "✗ FAIL") + " (Expected: $" + expectedPrice
					+ ", Actual: $" + actualPrice + ")");
			...
			softAssert.assertEquals(actualQty, expectedQty, "Product " + (i + 1) + " quantity mismatch!");
			System.out.println("Quantity: " + (qtyMatches ? "✓ PASS" : "✗ FAIL") + " (Expected: " + expectedQty
					+ ", Actual: " + actualQty + ")");
		}
